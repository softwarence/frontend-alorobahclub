...
export default function CardList({ items }: { items: item[] }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-2 max-w-[1800px] mx-auto">
      {items.map((item) => (
        <div
          key={item.id}
          className="relative w-full md:h-96 aspect-square rounded-xl overflow-hidden shadow-lg cut-corner group"
        >
          {/* Image */}
          <Image
            src={item.image}
